	...
	*addr = reinterpret_cast<void*>(out_addr);
	...
	if (out_addr == 0) {
		...
		std::printf("KYTY_DIAG mapdirect-outaddr0 in_addr=0x%016llx len=0x%016llx dmem=0x%016llx "
		            ...
		            consumed_reservation ? 1 : 0, shared_reason);
		std::fflush(stdout);
		if (consumed_reservation) {
			g_virtual_ranges->Add(in_addr, len, 0, 0, 0, VirtualRangeType::Reserved,
			                      consumed_range.name);
	...
	if (g_virtual_ranges->Query(vaddr, 0, &range) && IsReservedRangeType(range.type)) {
		...
		if (size != 0 && g_virtual_ranges->ConsumeReserved(block, size, range.type)) {
			...
			const bool range_added =
			    backend_mapped && g_virtual_ranges->Add(block, size, 0, CPU_RW_PROTECTION, 0,
			                                            VirtualRangeType::Flexible, range.name, true);
			if (!range_added) {
				if (backend_mapped) {
					GpuAccessMode rollback_gpu_mode = GpuAccessMode::NoAccess;
				...
				// Undo the commit so the block returns to its placeholder/free state before the
				// reservation record goes back into the virtual-range table.
				g_guest_address_space->ReleaseCommitted(block, size);
				g_virtual_ranges->Add(block, size, 0, 0, 0, range.type, range.name, false);
				return false;
			}

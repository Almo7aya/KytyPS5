...
// Decodes CB_COLOR#_CLEAR_WORD0/1. This only applies to a DCC_CLEAR_CODE_REGISTER clear; the
// constant-encoded codes carry their colour in the code itself and ignore these registers.
static bool DecodeColorClearRegisters(const HW::RenderTarget& rt, vk::Format format,
...
void RenderExecutor::ResolveRenderColorTarget(uint64_t submit_id, RenderCommandBuffer& buffer,
	...
	// normal color write path, not a render-target clear. A DCC fast clear is consumed later, once
	// the matching metadata fill has recorded a clear code for this surface.
	r.color_clear_enable = false;
	r.color_clear_value  = {};
	...
	if (rt.info.dcc_compression_enable && rt.dcc_addr.addr != 0) {
		// Host images are always decompressed, so the DCC allocation is not emulated as real
		// metadata. It is kept only as logical fast-clear state: a guest fill of that allocation
		// records a clear code, which AcquireRenderTargets turns into an attachment clear.
		desc.info.metadata.range = {rt.dcc_addr.addr, 0};
		desc.info.metadata.kind  = ImageMetadataKind::Dcc;
		// Only used when the recorded code is DCC_CLEAR_CODE_REGISTER.
		(void)DecodeColorClearRegisters(rt, target_format.format, r.color_clear_value);
	}

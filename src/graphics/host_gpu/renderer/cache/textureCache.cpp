...
float DecodeUnrealLogColor(float encoded) {
	// UE addresses its combined color-grading LUT in logarithmic scene-color space. A raw RGB
	// identity cube would therefore return the logarithmic address and crush ordinary scene colors.
...
float NeutralFilmicToSrgb(float linear) {
	// Supply only the essential log decode, neutral filmic highlight rolloff, and display transfer;
	// game-specific grading remains absent until the geometry-shader CombineLUT pass is supported.
	const float mapped =
...
std::vector<uint32_t> BuildNeutralColorGradingLut() {
	std::vector<uint32_t> lut(ColorGradingLutSize * ColorGradingLutSize *
	                          ColorGradingLutSize);
	for (uint32_t blue = 0; blue < ColorGradingLutSize; ++blue) {
		for (uint32_t green = 0; green < ColorGradingLutSize; ++green) {
			for (uint32_t red = 0; red < ColorGradingLutSize; ++red) {
				const auto encode = [](uint32_t value) {
					const float log_color = static_cast<float>(value) /
					                        static_cast<float>(ColorGradingLutSize - 1u);
				...
				lut[(blue * ColorGradingLutSize + green) * ColorGradingLutSize + red] =

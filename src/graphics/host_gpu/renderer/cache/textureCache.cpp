...
ImageId TextureCache::FindImage(ImageDesc& desc, bool exact_format) {
	...
	{
		std::scoped_lock lock {m_lock};
		// Exact backing matches share the start address, so their owners are all
		// present on the first page regardless of the texture's size.
		for (const auto id: FindImagesInRegion(desc.info.data.address, 1, false)) {

...
bool RenderExecutor::PrepareDrawRenderState(uint64_t submit_id, RenderCommandBuffer& buffer,
	...
	//
	// The render-target mask is what distinguishes a live slot from a stale register left behind by an earlier
	// pass, so attachment has to stay gated on it.
	//
	// Attaching every slot that merely holds a base address was tried and is wrong: it pulls in stale targets
	// from previous passes, and since the render area is bounded by the attachments, the framebuffer shrinks to
	// the smallest one - the game rendered cropped and never got past the loading screen. It also invalidated
	// the reasoning behind it. The `[mrt-drop]` lines that motivated the change were slots that switch itself
	// had forced Kyty to attempt; with the mask gate in place a slot is only tried when
	// `mask != 0 && addr != 0`, which is exactly when ResolveRenderColorTarget cannot bail, so slots are not
	// dropped and the packed indices never shift. See docs section 4.1k.
	//
	//   KYTY_STRICT_MRT=1 - skip a draw if a requested slot ever does fail to resolve, rather than letting the
	//                       later slots shift down an index. Kept as a guard, not as a fix.

...
bool MemoryTracker::IsRegionCpuModifiedOnly(uint64_t vaddr, uint64_t size) {
	CheckNotInUploadCallback();
	...
	const bool gpu_modified =
	    Iterate<true>(vaddr, size, [&](RegionManager* manager, uint64_t offset, uint64_t bytes) {
		    ...
		    cpu_modified |= manager->IsModified<DirtySource::Cpu>(offset, bytes);
		    return false;
	    });
	return cpu_modified && !gpu_modified;
}

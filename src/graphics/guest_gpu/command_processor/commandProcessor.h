...
public:
	...
	void SynchronizeGpu();
	// Lands any end-of-pipe value write still queued behind outstanding occlusion-query results
	// that targets the given range. For packets that read guest memory.
	void SynchronizeEndOfPipeWrites(uint64_t address, uint64_t size);

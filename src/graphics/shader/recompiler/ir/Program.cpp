...
static bool PhiEdgeReaches(const Program& program, const Block* owner, const Block* predecessor,
	...
	for (const auto& inst: *owner) {
		...
		for (size_t i = 0; i < inst.NumArgs(); ++i) {
			if (inst.PhiBlock(i) == predecessor) {
				incoming.emplace(&inst, inst.Arg(i));
			}
		}
	}
	...
	while (!pending.empty()) {
		...
		if (block == program.block_info.end()) {
			return true;
		}
		...
		if (term.kind == CFG::TerminatorKind::Return) {
			continue;
		}
		...
		if (condition != false) {
			pending.push_back(term.true_block);
		}
		if (condition != true) {
			pending.push_back(term.false_block);
		}
...
Value ResolveResourcePhi(const Program& program, Value value, uint32_t pc, uint32_t depth) {
	...
	if (phi == nullptr || phi->GetOpcode() != ValueOpcode::Phi || depth >= 64) {
		return value;
	}
	...
	for (size_t i = 0; i < phi->NumArgs(); ++i) {
		if (!PhiEdgeReaches(program, owner, phi->PhiBlock(i), pc)) {
			continue;
		}

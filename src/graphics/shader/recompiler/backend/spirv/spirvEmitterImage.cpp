...
void EmitImage(ValueEmitContext& ctx, const IR::Inst& inst) {
	...
	if ((op == IR::ValueOpcode::ImageSampleRaw || op == IR::ValueOpcode::ImageRead) &&
		...
		for (uint32_t iteration = 0; iteration < image.indirect_search_iterations; iteration++) {
			...
			state.builder.AddFunction(spv::OpSelect, TypeU32(state), probe, active, mid, ConstantU32(state, 0u));
			...
			state.builder.AddFunction(spv::OpSelect, TypeU32(state), next_selected, match, candidate, selected);
			...
			state.builder.AddFunction(spv::OpSelect, TypeU32(state), next_low, take_upper,

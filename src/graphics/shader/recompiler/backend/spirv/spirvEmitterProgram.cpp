...
bool EmitIndirectBufferInstruction(ValueEmitContext& ctx, const IR::Inst& inst) {
		...
		if (buffer.indirect_root == memory.resource) {
			...
			for (uint32_t iteration = 0; iteration < buffer.indirect_search_iterations;
				...
				selected = Select(state, TypeU32(state),
				                  Binary(state, spv::OpLogicalAnd, TypeBool(state), active, equal),
				                  candidate, selected);
...
void EmitDirectInstruction(ValueEmitContext& ctx, const IR::Inst& inst) {
	if (EmitIndirectBufferInstruction(ctx, inst)) return;

...
**Mechanism map (partly confirmed):** the GpuWorker (CP-thread fault) needs a texture readback and waits for the worker's `Ready` (`textureCache.cpp:425-435`). The readback worker is inside `rb_download`. Its wait is one of: **(a)** `g_tiler_mutex` — a *global* tiler lock held across `command.WaitForFence()` by `GpuTile`/`GpuDetile` (`gpuTiler.cpp:520/511`, `482`), or **(b)** the tiler's Vulkan fence never signaling (`command.WaitForFence()`), i.e. the queue can't retire the detile job. Which of (a)/(b) is **UNVERIFIED** — the `gpu_tile_lock`/`gpu_tile_fence` scopes were added to answer it; the run with them had not yet been executed when this document was written.

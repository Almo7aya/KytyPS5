...
So the cycle is: **worker (holding `m_resource_mutex` + `m_fault_mutex`, in `TextureCache::InvalidateMemory` Invalidate phase) → waits in `ReadbackWorker::Request` for the readback worker; readback worker → stuck completing the readback.**

**Session-3 update 2 (CONFIRMED — the GPU-fence theory above is REFUTED).** Added a bounded `CommandBuffer::WaitForFenceOnly` (`context.cpp:349`): waits 4 s, and on timeout prints `KYTY_DIAG fence-stall ... status=<getFenceStatus>` (distinguishes GPU device-lost/TDR from a merely-queued submission), then resumes the infinite wait. Reproduced the deadlock (run `fst2`, page `0x20704e0000`, held 7 s) and **the `fence-stall` line did NOT print** — so the readback worker is **not** in any `CommandBuffer::WaitForFence`; the GPU copy fence is not the blocker. Symbolized readback-worker stack (`fst2`): `ReadbackWorker::DownloadColorImage → WriteBacking → DirectMemoryBacking::TryTransferBacking → GpuTile` (and `BufferCache::UnmapMemory`). So after the image download, `WriteBacking` re-tiles the guest backing (`TryTransferBacking → GpuTile`) and/or unmaps overlapping buffers, and it is stuck there — NOT on a GPU fence, NOT on `g_tiler_mutex`/tiler-fence (state would be `gpu_tile_*`), NOT on `cache.m_lock` at line 839 (`TrackingSpinLock` is a pure spin → would burn CPU, not park). The full stall dump shows **no other thread holding the contended resource** (all others are `cond`/`gpu_pause_lock`/`gpu_idle`, i.e. before any cache-lock acquisition). The `state=rb_download` vs stack-shows-`GpuTile` mismatch is unresolved (either a stale scan frame or a scope-restore gap).
- **Next probe:** add explicit scopes inside `DirectMemoryBacking::TryTransferBacking`, `BufferCache::UnmapMemory`, and the tiler's non-`CommandBuffer` waits to pin the exact primitive the readback worker parks on (it is an OS wait that is none of the already-scoped ones).
- **Fix direction (unchanged in spirit):** the CP-thread fault must not synchronously wait for the async readback worker while holding `m_resource_mutex`/`m_fault_mutex` and cache locks; either do the readback inline on the CP thread (it owns the scheduler post-`GraphicsRunFinishScheduler`), or ensure `WriteBacking`/`TryTransferBacking` on the readback worker cannot need a lock/tiler resource the faulting worker path holds.

_Superseded session-3 update 1 (kept for history):_ earlier symbolization suggested the readback worker was on a GPU download-copy fence in `DownloadColorImage`; the `fence-stall` probe above refutes that.

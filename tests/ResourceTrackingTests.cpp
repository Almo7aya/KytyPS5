...
void TestResourceSpecializationIsTypedAndTransactional() {
	...
	unknown_program.blocks[0].instructions = {
	    ImageUse(0x14, Opcode::ImageLoad, ResourceKind::Image, Decoder::ImageDimension::Unknown)};
	Prepare(unknown_program);
	Check(unknown_program.info.images.size() == 1 &&
	          unknown_program.info.images[0].dimension == Decoder::ImageDimension::Unknown,
	...
	Check(SpecializeResources(unknown_program, unknown_snapshot, &error) &&
	          ValidateResourceSpecialization(unknown_program, unknown_snapshot, &error) &&
	          unknown_program.info.images[0].kind == ResourceKind::Image &&
	          unknown_program.info.images[0].dimension == Decoder::ImageDimension::Dim2D,
